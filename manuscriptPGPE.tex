...
% insert abstract here
  The chemistry of lipid headgroups, the water facing components of cell membranes that regulate cell functions via lipid--protein interactions, varies between organisms and organelles. 
  In biological membranes at physiological conditions,
  lipid molecules sample an ensemble of conformations.
  However, the individual sets of conformations in the ensemble and their dependence on lipid types have not yet been experimentally determined. 
  It then remains unclear how the interactions with membrane proteins
  are related to the structural configurations of the lipids. 
  Here, we combine solid-state NMR experiments and molecular dynamics simulations from the NMRlipids open collaboration to resolve the conformational ensembles of the headgroups of several key lipid types in the liquid phase under various biologically relevant conditions. Interpretation of the NMR observables using the plethora of simulation data collected in the NMRlipids project suggests that all lipid headgroups sample a wide range of conformations in both neutral and charged cellular membranes.
  Only the overall orientations of the headgroups 
  measured by P-N/C$_\gamma$ vector 
  is affected by membrane charge
  and is related to the C$_\beta$ headgroup order parameter. 
  Differences in the headgroup chemistry between different lipid types manifest
  in the conformation probability distributions, 
  nevertheless all lipid types can access almost any of the possible conformations.
  This is in line with our analysis of protein-bound lipids from the protein data bank (PDB), 
  which suggests that all lipid types can bind to proteins in a wide range of conformations independently of their headgroup chemistry. Therefore, the selective adsorption of peripheral proteins to membranes and binding of lipids to transmembrane proteins are likely regulated by specific protein--lipid interactions rather than conformational restrictions of the bilayer lipids 
  justifying the use of simplified coarse-grained models,
  which account for the intermolecular interactions 
  as well as for the entropic effects 
  in their effective empirical potentials.
...
only from specific non-covalent lipid--protein interactions.
...
Zwitterionic PC and PE lipids are the most common lipids in eukaryotes and bacteria, respectively \cite{vanmeer08,sohlenkamp16}.
...
The similarity of glycerol backbone and headgroup order parameters in model membranes and bacteria \cite{gally81,scherer87,seelig90}
...
Only the structures determined using X-ray crystallography or Cryo-EM with a resolution higher than 3.2~\text{\AA} were used in the analysis. 
Copies of the lipids from symmetric structures of multimeric proteins 
that exhibited identical configurations 
(dihedral angles matching within 3 degrees)
were counted only once
to aviod putting an artificially large weight on the conformation.
...
The pairs of different protein-attached/interacting lipids 
with similar configuration
were searched by comparing the following torsion angles
A LIST OF THE COMPARED ANGLES.
Pairs of lipids were selected as having similar structure
if their torsion angles agreed within 30$^/circ$. 
Only the crystal structures structures with a resolution higher than 2.5~\text{\AA} were used. 
...
\begin{figure*}[bt]
  \centering
   ...
   \caption{\label{structures}
     ...
     \textbf{D)} Distributions of P--N/C$_\gamma$ vector angle with respect to membrane normal without entropic correction.
  }
\end{figure*}
...
This means that compared to the other phospholipids 
the orientation of the C$_\beta$–H bond in the PG headgroup
has a small preference for being parallel to the membrane normal.
At the same time,
increasing the $\beta$-carbon order parameters in the phospholipids
yields corresponding systematic shifts 
of the P-N/C$_\gamma$ vector angle distributions 
towards configurations perpendicular to the membrane normal 
(Fig \ref{structures}~B and D). 

The glycerol backbone order parameters are similar for all the lipid types, although they move slightly toward positive values (closer to zero) in the order PC $<$ PE $<$ PS $<$ PG. Only minor differences between PC and PE headgroups are observed.
...
and P--N/C$_\gamma$ vector dipole may be artificial as the $\beta$-carbon order parameter in PC is too negative even in the best available force field, thereby not being equal to the order parameter in PE as observed in experiments \cite{botan15}.
...
MD simulations can represent such conformational ensembles 
with accurate enough force fields.
...
dihedrals.
The choline region remains essentially unchanged and only minor changes are observed in the remaining headgroup dihedrals even at a surfactant molar fraction of 0.47 (Fig S\ref{HGorderparametersPCvsSURFdihedrals} in SI).
...
This provides further justification for using simplified coarse-grained models,
which account for the intermolecular interactions 
as well as for the entropic effects 
in their effective empirical potentials. \cite{LipidFingerprints2018}
