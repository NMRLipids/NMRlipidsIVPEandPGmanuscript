...
but their relation to lipids in bulk membranes in the liquid lamellar phase remains unclear \cite{marsh13b}. Similarly to disordered proteins~\cite{sormanni17}, lipids sample conformational ensemble which is composed of the set of individual structures whose probability to occur is determined by the Boltzmann distribution. In liquid bilayer state, individual phosphatidylcholine lipid samples the conformational ensemble within nanosecond timescale~\cite{ferreira15}. The most accurate experimental information on conformational ensembles of lipids
in this biologically relevant phase are typically derived from NMR experiments, particularly from the
C--H bond order parameters, $S_\mathrm{CH}=\frac{3}{2} \langle \cos^2 \theta -1 \rangle$, where $\theta$ is the angle between C-H bond and membrane normal, and the average is taken over conformational ensemble of lipids~\cite{seelig77c,davis83,Semchyschyn04}.
