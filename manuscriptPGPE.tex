...
\title{Conformational plasticity of lipid headgroups in cellular membranes and protein--lipid complexes} %Title of paper
...
% insert abstract here
  The chemistry of lipid headgroups, the water-facing interface of cellular membranes, regulates cell functions via lipid--protein interactions. In the physiologically relevant liquid state,
  ...
  However, the distributions of conformations in such ensembles and their lipid-type dependence have not yet been experimentally determined: 
  ...
  Here, we combine solid-state NMR experiments and molecular dynamics simulations from the NMRlipids Project to resolve the conformational ensembles of headgroups of four key lipid types in various biologically relevant conditions. We find all lipid headgroup types to sample a wide range of conformations in both neutral and charged cellular membranes.
  Differences in the headgroup chemistry do manifest
  in probability distributions of conformations, but all four lipid types can access almost the whole conformation range.
  Together with the analysis of 894 protein-bound lipid structures from the Protein Data Bank (PDB), this suggests that lipids can bind to proteins in a wide range of conformations, which are not limited by the headgroup chemistry. Therefore, the selective adsorption of peripheral proteins to membranes and the binding of lipids to transmembrane proteins are likely to be regulated by specific protein--lipid interactions, not by any inherent conformational restrictions of the lipids.
...
Here, we use natural abundance $^1$H-$^{13}$C solid-state NMR experiments %(enabling the determination of order parameter signs)
...
\begin{figure*}[bt]
  \centering
   ...
   \caption{\label{structures}
     ...
     %For a detailed comparison of all the XX force-fields used with the $S_{\rm{CH}}$ values measured please read the supplementary information.
     ...
     Angles corresponding to energies above 8~$k_\mathrm{B}T$ are not shown because they are not observed in simulations.
...
C--H bond order parameters $S_\mathrm{CH}$ from NMR experiments %in this and previous studies \cite{wohlgemuth80,buldt81,seelig87}
